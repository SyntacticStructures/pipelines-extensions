...
function SetupSSH($key_name) {
  ...
  execute_cmmand "ls $(Join-Path $env:USERPROFILE -ChildPath `".ssh`")"
...
function DeployApplication() {
  ...
  foreach ($vm_target in $vm_targets) {
    execute_command "ssh -i C:\Users\ContainerAdministrator\.ssh\myVM -vvv root@2.tcp.ngrok.io -p 18205  -o StrictHostKeyChecking=no `"ls /`""
#    ssh $vm_targets "ls /"
  }
}
